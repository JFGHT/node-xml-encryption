...
function decryptKeyInfo(doc, options) {
  ...
  switch (keyEncryptionAlgorighm) {
    case 'http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p':
      return decryptKeyInfoWithScheme(encryptedKey, options, 'pkcs1_oaep');
    case 'http://www.w3.org/2001/04/xmlenc#rsa-1_5':
      return decryptKeyInfoWithScheme(encryptedKey, options, 'pkcs1');
    default:
...
function decryptKeyInfoWithScheme(encryptedKey, options, scheme) {
  ...
  var private_key = new NodeRSA(options.key, 'pkcs1', { encryptionScheme: scheme });
  var decrypted = private_key.decrypt(key);
